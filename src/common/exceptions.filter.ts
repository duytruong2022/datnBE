...
import { customErrorMessagePrefix, HttpStatus } from './constants';
...
const translateErrorValidator = async (
    ...
    const errorMessages = await Promise.all(
        errors.map(async (error: ValidationErrorItem) => {
            ...
            // translate message
            let resultMessage = '';
            if (message.startsWith(customErrorMessagePrefix)) {
                resultMessage = await i18n.translate(
                    message.replace(customErrorMessagePrefix, ''),
                    {
                        args: (context as any)?.args,
                    },
                );
            } else if (context.name) {
                resultMessage = await i18n.translate(context.name, {
                    args: (context as any)?.args,
                });
            } else {
                resultMessage = await i18n.translate(key, {
                    args: (context as any)?.args,
                });
            }
            return {
